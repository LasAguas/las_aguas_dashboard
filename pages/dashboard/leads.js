...
function MeetingsCalendarSection() {
  ...
  return (
    <Section
      ...
      subtitle="View upcoming meetings and block unavailable times. Click empty slots to block, click booked/blocked to remove."
      defaultOpen={true}
      right={
        <div className="text-xs text-[#33286a]/80">
        ...
        {allHours.map((hour) => (
          <div key={hour} className="grid grid-cols-8 border-b border-black/10 last:border-b-0">
            ...
            {currentWeek?.days.map((day, dayIdx) => {
              ...
              // No slot available for this day/time
              if (!hasSlot) {
                return (
                  <div
                    key={`${day.dateStr}-${hour}`}
                    className="p-1 border-l border-black/10 bg-gray-50"
                  />
                );
              }
              ...
              if (isPast && !slot) {
                return (
                  <div
                    key={`${day.dateStr}-${hour}`}
                    className="p-1 border-l border-black/10 bg-gray-100"
                  />
                );
              }

              // Has a booking or block
              if (slot) {
