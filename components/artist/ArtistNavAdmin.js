...
const NAV = [
  ...
  {
    href: "/dashboard/menu",
    label: "Back Home (Regular Dashboard)",
    icon: "/icons/artist-nav/Home-icon.png",
  },
  {
    href: "/dashboard/insights-admin",
...
export default function ArtistNavAdmin({ forceDesktopOpen = false, desktopOpen, setDesktopOpen }) {
  ...
  useEffect(() => {
    ...
    async function loadPending() {
      try {
        ...
        console.error("[ArtistNavAdmin] pending count error", e);
        if (!cancelled) setPendingCount(0);
      }
    }
